...
<?php // El filtro de variedad activo viaja como data-*: no es codigo, asi que la CSP no lo restringe. ?>
<body data-variedad-filtro="<?= $f_variedad ?: '' ?>">
<nav>
    <div class="n-logo">
                        ...
                        <td data-label="Cupo Semanal">
                            ...
                            <div style="display:flex; flex-direction:column; gap:0.25rem;">
                                ...
                                <form method="POST" style="display:inline-flex; align-items:center; gap:0.25rem; margin-top:0.15rem;">
                                    ...
                                    <input type="number" name="cupo_semanal" value="<?= (int)$cupo ?>" min="0" max="<?= ReglasPortal::CUPO_MAXIMO ?>" step="<?= ReglasPortal::CUPO_MULTIPLO ?>" oninput="if(this.value !== '' && parseFloat(this.value) > <?= ReglasPortal::CUPO_MAXIMO ?>) this.value = <?= ReglasPortal::CUPO_MAXIMO ?>; if(this.value !== '' && parseFloat(this.value) < 0) this.value = 0;" style="width:75px; height:1.6rem; padding: 0 0.4rem; font-size: 0.72rem; border-radius: 6px; border:1px solid var(--border); background:var(--input-bg); color:var(--fg); text-align:right; outline:none;">
...
<script src="<?= APP_URL ?>/assets/js/portal_dashboard.js?v=<?= APP_VERSION ?>"></script>
</body>
</html>
