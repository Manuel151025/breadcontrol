...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['verificar_usuario'])) {
        ...
        if (!$usuario_input) {
            ...
            $stmt = $pdo->prepare("SELECT id_cliente, nombre, email, pin_recuperacion FROM cliente WHERE usuario = ? AND activo = 1");
            ...
            if (!$cliente) {
                ...
                if (empty($cliente['email'])) {
                    ...
                    $pdo->prepare("UPDATE cliente SET codigo_recuperacion = ?, codigo_expira = ? WHERE id_cliente = ?")
                        ->execute([$codigo, $expira, $cliente['id_cliente']]);
    ...
    elseif (isset($_POST['verificar_codigo'])) {
        ...
        if (!$cid || !preg_match('/^\d{6}$/', $codigo)) {
            ...
            $stmt = $pdo->prepare("SELECT codigo_recuperacion, codigo_expira FROM cliente WHERE id_cliente = ?");
            $stmt->execute([$cid]);
            $row = $stmt->fetch();
            if (!$row || $row['codigo_recuperacion'] !== $codigo) {
                ...
                $pdo->prepare("UPDATE cliente SET codigo_recuperacion = NULL, codigo_expira = NULL WHERE id_cliente = ?")
                    ->execute([$cid]);
                $_SESSION['recover_pin_ok'] = true;
                $paso = 3;
            }
        } else {
            $stmt = $pdo->prepare("SELECT pin_recuperacion FROM cliente WHERE id_cliente = ?");
            $stmt->execute([$cid]);
    ...
    elseif (isset($_POST['cambiar_pass'])) {
        ...
        if (!$cid || !$pin_ok) {
            ...
            $pdo->prepare("UPDATE cliente SET contrasena_hash = ? WHERE id_cliente = ?")
                ->execute([password_hash($nueva, PASSWORD_DEFAULT), $cid]);
            unset($_SESSION['recover_cid'], $_SESSION['recover_cnombre'], $_SESSION['recover_cemail'],
                  $_SESSION['recover_metodo'], $_SESSION['recover_pin_ok']);
