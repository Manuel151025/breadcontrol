    ...
    if ($nombre && $usuario && $contrasena) {
        if (!preg_match('/^[a-z0-9_]+$/', $usuario)) {
            ...
            if ($stmt->fetch()) {
                ...
                $stmt = $pdo->prepare("INSERT INTO cliente (nombre, tipo, telefono, usuario, contrasena_hash, es_aprendiz, activo, fecha_creacion) VALUES (?, ?, ?, ?, ?, ?, 1, NOW())");
                try {
                    $stmt->execute([$nombre, $tipo, $telefono, $usuario, $hash, $es_aprendiz]);
...
<div class="login-wrap">
  <div class="glass-card">
    ...
    <?php if ($success): ?>
    <div class="msg-success" style="flex-direction:column; padding:2rem; text-align:center;">
    ...
    <form method="POST" style="width:100%; display:flex; flex-direction:column; gap:1.3rem;">
        ...
        <div class="field" style="display:flex; align-items:center; gap:0.5rem; background: rgba(255,167,38,.1); padding: 0.8rem; border-radius: 8px; border: 1px solid rgba(255,167,38,.3);">
            ...
            <label for="es_aprendiz" style="margin-bottom:0; color: var(--honey); font-size: 0.85rem;">Soy aprendiz SENA (Mis pedidos se cobrarán a Tienda ADSO)</label>
        </div>
