...
$stmt_cli = $pdo->prepare("SELECT tipo, es_aprendiz FROM cliente WHERE id_cliente = ?");
$stmt_cli->execute([$cliente_id]);
$cliente_info = $stmt_cli->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_pedido'])) {
    ...
    if (empty($fecha_entrega)) {
        $error = 'Debes seleccionar una fecha de entrega.';
    } elseif ($fecha_entrega < $hoy && $edit_id == 0) {
        $error = 'La fecha de entrega no puede ser en el pasado.';
    } elseif ($fecha_entrega > date('Y-m-d', strtotime('+3 months'))) {
        ...
        try {
            ...
            if (empty($cart_validado)) {
                throw new Exception("El carrito no contiene productos válidos o las cantidades son incorrectas.");
            }
            
            if ($edit_id > 0) {
                $stmt_chk = $pdo->prepare("SELECT id_pedido, estado, fecha_entrega, id_cliente FROM pedido_cliente WHERE id_pedido = ? AND (id_cliente = ? OR id_creador = ?)");
                $stmt_chk->execute([$edit_id, $cliente_id, $cliente_id]);
                $ped_chk = $stmt_chk->fetch();
                ...
                $id_cli_edit = $ped_chk['id_cliente'];
                if ($es_aprendiz) {
                    $pedido_para = $_POST['pedido_para'] ?? 'adso';
                    if ($pedido_para === 'adso') {
                        $stmt_adso = $pdo->prepare("SELECT id_cliente FROM cliente WHERE nombre LIKE '%Tienda ADSO%' AND tipo='tienda' LIMIT 1");
                        $stmt_adso->execute();
                        $id_adso = $stmt_adso->fetchColumn();
                        if ($id_adso) $id_cli_edit = $id_adso;
                    } else {
                        $id_cli_edit = $cliente_id;
                    }
                }
                $stmt_ped = $pdo->prepare("UPDATE pedido_cliente SET fecha_entrega = ?, total_estimado = ?, id_cliente = ? WHERE id_pedido = ?");
                $stmt_ped->execute([$fecha_entrega, $total_dinero, $id_cli_edit, $edit_id]);
                $id_pedido = $edit_id;
                
                $pdo->prepare("DELETE FROM pedido_cliente_detalle WHERE id_pedido = ?")->execute([$id_pedido]);
            } else {
                $id_cli_insert = $cliente_id;
                $id_creador_insert = $cliente_id;
                $pedido_para = $_POST['pedido_para'] ?? 'adso';

                if ($es_aprendiz && $pedido_para === 'adso') {
                    $stmt_tienda_adso = $pdo->prepare("SELECT id_cliente FROM cliente WHERE nombre LIKE '%Tienda ADSO%' AND tipo='tienda' LIMIT 1");
                    $stmt_tienda_adso->execute();
                    $id_tienda_adso = $stmt_tienda_adso->fetchColumn();
                    if ($id_tienda_adso) {
                        $id_cli_insert = $id_tienda_adso;
                    }
                }
                
                $stmt_ped = $pdo->prepare("INSERT INTO pedido_cliente (id_cliente, id_creador, fecha_entrega, total_estimado) VALUES (?, ?, ?, ?)");
                $stmt_ped->execute([$id_cli_insert, $id_creador_insert, $fecha_entrega, $total_dinero]);
                $id_pedido = $pdo->lastInsertId();
            }
            
            $stmt_det = $pdo->prepare("INSERT INTO pedido_cliente_detalle (id_pedido, id_variedad, cantidad, precio_unitario, napa, bonificacion) VALUES (?, ?, ?, ?, 0, 0)");
            foreach ($cart_validado as $item) {
                $stmt_det->execute([$id_pedido, $item['id_variedad'], $item['cantidad'], $item['precio']]);
            }
            
            // Determinar el cliente real asignado al pedido
            $order_cliente_id = ($edit_id > 0) ? $id_cli_edit : $id_cli_insert;
            $stmt_cli_tipo = $pdo->prepare("SELECT tipo FROM cliente WHERE id_cliente = ?");
            $stmt_cli_tipo->execute([$order_cliente_id]);
            $order_cli_tipo = $stmt_cli_tipo->fetchColumn();
            $es_tienda_actual = ($order_cli_tipo === 'tienda');

            // Validar de antemano el costo total de los productos de bonificación/ñapa
            ...
            // Guardar detalles de bonificación/ñapa validando precio real
            if (!empty($bonif_items)) {
                foreach ($bonif_items as $bi) {
                    $cant = (int)($bi['cantidad'] ?? 0);
                    $id_var = (int)($bi['id_variedad'] ?? 0);
                    ...
                    if ($cant > 0 && $id_var > 0) {
                        ...
                        if ($precio_real !== false) {
                            $pdo->prepare("INSERT INTO pedido_cliente_detalle (id_pedido, id_variedad, cantidad, precio_unitario, napa, bonificacion) VALUES (?, ?, 0, ?, ?, ?)")
...
if ($edit_id > 0) {
    $stmt_chk = $pdo->prepare("SELECT * FROM pedido_cliente WHERE id_pedido = ? AND (id_cliente = ? OR id_creador = ?) AND estado = 'pendiente'");
    $stmt_chk->execute([$edit_id, $cliente_id, $cliente_id]);
    $ped_edit = $stmt_chk->fetch(PDO::FETCH_ASSOC);
    if ($ped_edit) {
        ...
        $rows = $stmt_det_edit->fetchAll(PDO::FETCH_ASSOC);
        foreach ($rows as $r) {
            if ($r['cantidad'] > 0) {
                $cart_preload[] = [
...
$categorias = $pdo->query("SELECT * FROM categoria_precio WHERE activo=1 ORDER BY precio_unitario")->fetchAll();
...
<body>
    <nav>
        <a href="dashboard.php" class="n-logo">
            ...
            <div><div class="n-logo-name">BreadControl</div><div class="n-logo-sub">Portal de Cliente</div></div>
        </a>
        <div class="n-right">
            ...
            <div class="panel-der">
                <form method="post" id="form-pedido">
                    ...
                    <div style="margin-bottom: 1rem;">
                        <label style="font-size: .75rem; font-weight: 700; color: var(--ink3); text-transform: uppercase; display:block; margin-bottom:.3rem;">Fecha de entrega solicitada</label>
                        <input type="date" name="fecha_entrega" min="<?= date('Y-m-d') ?>" max="<?= date('Y-m-d', strtotime('+3 months')) ?>" value="<?= $ped_edit ? $ped_edit['fecha_entrega'] : date('Y-m-d') ?>" required style="width: 100%; padding: .6rem; border: 1px solid var(--border); border-radius: 8px; font-family: inherit; font-size:.9rem; color: var(--ink);">
                    </div>
    ...
    var esTienda = <?= ($es_tienda || ($es_aprendiz && $pedido_para_actual === 'adso')) ? 'true' : 'false' ?>;
    var bonifCredito = 0;
    ...
    function renderBonifVars() {
        ...
        var html = '';
        var colSoft = esTienda ? '#64b5f6' : '#e4a565';
    ...
    function updateBonifStatus(triggerInput = null) {
        ...
        // Primera pasada: calcular gastado
        inputs.forEach(inp => {
        ...
        // Limitar si se pasó del crédito disponible
        if (triggerInput && gastado > bonifCredito) {
            ...
            if (prTrigger > 0) {
                ...
                return updateBonifStatus(); // Recalcular con el nuevo valor
            }
        }

        // Segunda pasada: construir array y total real
        gastado = 0; 
        inputs.forEach(inp => {
            var val = parseInt(inp.value)||0;
            var pr = parseFloat(inp.dataset.bonifPrecio)||0;
        ...
        // Prevent form submission if bonif exceeded
        document.getElementById('btn-pedido').disabled = (gastado > bonifCredito);
    }

    // Inicializar si hay carrito precargado
    document.addEventListener("DOMContentLoaded", function() {
        if (cart.length > 0) {
            renderCart();
            // Pre-seleccionar la primera pestaña para mostrar catálogo
            let firstTab = document.querySelector('.price-tab');
            if (firstTab) firstTab.click();
            
            // Esperar a que se carguen las variedades y aplicar precarga
