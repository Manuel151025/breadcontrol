...
if (!empty($pedido['id_pago_activo'])) {
    $stmt_pago = $pdo->prepare("SELECT * FROM pago_pedido WHERE id_pago = ?");
    $stmt_pago->execute([$pedido['id_pago_activo']]);
    $pago_activo = $stmt_pago->fetch();
...
// Titular del negocio (para mostrar al cliente)
$stmt_cfg = $pdo->query("SELECT nequi_titular, nequi_link_pago FROM configuracion LIMIT 1")->fetch();
$titular_negocio = $stmt_cfg['nequi_titular'] ?? '';
$nequi_link_pago = $stmt_cfg['nequi_link_pago'] ?? '';

// Verificar si el pedido pertenece a un cliente tipo tienda
$stmt_tipo = $pdo->prepare("SELECT c.tipo, c.nombre FROM pedido_cliente p JOIN cliente c ON p.id_cliente = c.id_cliente WHERE p.id_pedido = ?");
$stmt_tipo->execute([$id_pedido]);
$row_tipo   = $stmt_tipo->fetch();
$orden_es_de_tienda = ($row_tipo['tipo'] === 'tienda');
$es_tienda          = $orden_es_de_tienda && ($pedido['id_cliente'] === $cliente_id);
$nombre_tienda      = $row_tipo['nombre'] ?? '';

// Verificar si TODOS los pedidos de esta tienda para esta fecha están confirmados
...
if ($es_tienda) {
    ...
    ");
    $stmt_pend->execute([$pedido['id_cliente'], $pedido['fecha_entrega']]);
    $pendientes_count  = (int)$stmt_pend->fetchColumn();
    $todos_confirmados = ($pendientes_count === 0);
}

// Reporte detallado por aprendiz (solo tiendas, misma fecha de entrega)
$reporte_por_aprendiz = [];
$total_general_reporte = 0;
if ($es_tienda) {
    ...
    ");
    $stmt_rep->execute([$pedido['id_cliente'], $pedido['fecha_entrega']]);
    foreach ($stmt_rep->fetchAll() as $row) {
        $ap = $row['aprendiz'];
        if (!isset($reporte_por_aprendiz[$ap])) $reporte_por_aprendiz[$ap] = [];
        $reporte_por_aprendiz[$ap][] = $row;
    }
    // Totales por pedido (para sumar el total general)
    $stmt_tot = $pdo->prepare("
        SELECT SUM(total_estimado)
        FROM pedido_cliente
        WHERE id_cliente = ? AND fecha_entrega = ? AND estado IN ('confirmado','pendiente')
    ");
    $stmt_tot->execute([$pedido['id_cliente'], $pedido['fecha_entrega']]);
    $total_general_reporte = (float)$stmt_tot->fetchColumn();
}
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        ...
        .info-item span { font-size: 0.75rem; color: var(--ink3); text-transform: uppercase; letter-spacing: 0.05em; font-weight: 700; }
        .info-item strong { font-size: 1rem; color: var(--ink); }
        ...
        /* Monto destacado para que el cliente sepa qué digitar */
...
<body>
    ...
    <div class="page">
        ...
        <div class="card">
            ...
            <div class="r-body">
                <?php if($dentro_limite && $pedido['estado'] === 'pendiente'): ?>
                ...
                <?php elseif ($estado_pago === 'parcial' && $pago_activo): ?>
                    ...
                    $stmt_sum->execute([$pedido['id_pago_activo']]);
                    $total_esperado = (float)$stmt_sum->fetchColumn();
                    $saldo_restante = $total_esperado - $total_pagado;
                    ?>
