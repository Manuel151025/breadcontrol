...
// Verificar que el pedido pertenece a este cliente (tienda)
$stmt = $pdo->prepare("
...
// Consulta del reporte agrupado por aprendiz y producto
...
$stmt_tot = $pdo->prepare("
    ...
    FROM pedido_cliente
    WHERE id_cliente = ? AND fecha_entrega = ? AND estado IN ('confirmado','pediente')
");
$stmt_tot->execute([$pedido['id_cliente'], $pedido['fecha_entrega']]);
$total_general = (float)$stmt_tot->fetchColumn();

$fecha_entrega_fmt = date('d/m/Y', strtotime($pedido['fecha_entrega']));
