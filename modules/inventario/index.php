...
// ── Eliminar múltiples insumos ───────────────────────────────
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['eliminar_seleccionados'])) {
    $ids = $_POST['ids_eliminar'] ?? [];
    if (!empty($ids)) {
        $placeholders = implode(',', array_fill(0, count($ids), '?'));
        $pdo->prepare("UPDATE insumo SET activo=0 WHERE id_insumo IN ($placeholders)")
...
<style>
  ...
  .filter-banner{background:rgba(198,113,36,.06);border:1px solid rgba(198,113,36,.18);border-radius:10px;padding:.5rem .85rem;font-size:.78rem;color:var(--c3);font-weight:600;display:flex;align-items:center;gap:.4rem;margin:.6rem .85rem .6rem;}
...
<div class="page">
  ...
  <div class="g-body">
    ...
    <div class="card">
      ...
      <div id="batch-bar" style="display:none;padding:.6rem 1rem;background:rgba(198,57,40,.06);border-top:1px solid rgba(198,57,40,.15);align-items:center;gap:.6rem;">
        <span style="font-size:.78rem;font-weight:700;color:#c62828;" id="batch-count">0 seleccionados</span>
        <button type="submit" name="eliminar_seleccionados" class="btn-act btn-del" style="padding:.35rem .7rem;font-size:.78rem;border-radius:8px;gap:.3rem;display:inline-flex;align-items:center;"
          onclick="return confirm('¿Eliminar los insumos seleccionados?')">
