...
if (isset($_GET['ajax_detalle_venta'])) {
    ...
    try {
        ...
        $det = $pdo->prepare("
            SELECT vd.id_variedad, vd.cantidad, vd.napa, vd.precio_unitario,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_venta'])) {
    ...
    if (!in_array($tipo_salida, ['venta','bonificacion','consumo_interno'])) $tipo_salida = 'venta';
    if (!$id_cat && $precio_custom <= 0) $msg_err = 'Selecciona una categoría de precio.';
    elseif ($cantidad<=0) $msg_err = 'La cantidad debe ser mayor a 0.';
        ...
        if (!$cat_data) {
            ...
            $bonificacion = 0; $napa = 0; $und_fisicas = $cantidad;
            ...
            } elseif ($tipo_salida === 'venta' && $id_cliente == 0 && $dar_napa && $napa_cant > 0) {
                $napa = $napa_cant; $und_fisicas = $cantidad + $napa;
            }
            ...
            if ($und_fisicas > $stock_disponible && $tipo_salida === 'venta' && $id_cat) {
                ...
                try {
                    ...
                    if ($bonificacion > 0) $msg_ok .= "<br>+ <strong>$bonificacion bonificadas 🏪</strong> = <strong>$und_fisicas entregadas</strong>";
                    if ($napa > 0) $msg_ok .= "<br>+ <strong>$napa de ñapa 🎁</strong> = <strong>$und_fisicas entregadas</strong>";
                    if ($tipo_salida === 'venta') $msg_ok .= "<br>Total: <strong>$".number_format($total,0,',','.')."</strong>";
                } catch (Exception $e) { $msg_err = 'Error: '.$e->getMessage(); }
            }
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_pedido'])) {
    ...
    if (empty($cart)) {
        ...
        try {
            $total_und = 0; $total_napa = 0; $total_dinero = 0;
            foreach ($cart as $item) {
                $total_und += (int)$item['cantidad'];
                $total_napa += (int)($item['napa'] ?? 0);
            ...
            if ($id_cliente > 0) {
                ...
                $tc->execute([$id_cliente]);
                if ($tc->fetchColumn() === 'tienda') {
                    $bonif_ped = (int)floor($total_und * 0.20);
                }
            }

            $und_totales = $total_und + $total_napa + $bonif_ped;
            ...
            $pdo->prepare("INSERT INTO venta (tipo_salida, id_cliente, id_usuario, fecha_hora, unidades_vendidas, precio_unitario, total_venta, unidades_bonificacion) VALUES ('venta',?,?,NOW(),?,0,?,?)")
                ->execute([
                    ...
                    $und_totales,
                    $total_dinero,
                    $total_napa + $bonif_ped
                ]);
            $id_venta = $pdo->lastInsertId();

            foreach ($cart as $item) {
                $pdo->prepare("INSERT INTO venta_detalle (id_venta, id_variedad, cantidad, napa, bonificacion, precio_unitario) VALUES (?,?,?,?,?,?)")
                    ->execute([$id_venta, (int)$item['id_variedad'], (int)$item['cantidad'], (int)($item['napa'] ?? 0), 0, (float)$item['precio']]);
            }

            // Guardar detalle de bonificación si existe
            ...
            if ($total_napa > 0) $msg_ok .= "<br>🎁 Ñapa: <strong>$total_napa</strong> unidades";
            if ($bonif_ped > 0) $msg_ok .= "<br>🏪 Bonificación tienda: <strong>$bonif_ped</strong> unidades";
            $msg_ok .= "<br>Total: <strong>$".number_format($total_dinero,0,',','.')."</strong>";
        } catch (Exception $e) { $msg_err = 'Error: '.$e->getMessage(); }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['editar_pedido'])) {
    ...
    if ($id_v && !empty($cart)) {
        try {
            ...
            // Delete old detalle and insert new
            $pdo->prepare("DELETE FROM venta_detalle WHERE id_venta=?")->execute([$id_v]);
            foreach ($cart as $item) {
                $pdo->prepare("INSERT INTO venta_detalle (id_venta, id_variedad, cantidad, napa, precio_unitario) VALUES (?,?,?,?,?)")
        ...
        } catch (Exception $e) { $msg_err = 'Error al editar: '.$e->getMessage(); }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['editar_venta'])) {
    ...
    if (!in_array($tipo, ['venta','bonificacion','consumo_interno'])) $tipo = 'venta';
    if ($id_v && $id_cat && $cant > 0) {
        $cat = $pdo->prepare("SELECT precio_unitario FROM categoria_precio WHERE id_categoria=?");
        $cat->execute([$id_cat]); $precio = (float)$cat->fetchColumn();
        $total = ($tipo === 'venta') ? round($precio * $cant, 2) : 0;
        $bonif_edit = 0; $und_edit = $cant;
        if ($tipo === 'venta' && $id_cli > 0) {
            $tc = $pdo->prepare("SELECT tipo FROM cliente WHERE id_cliente=?");
            $tc->execute([$id_cli]);
            if ($tc->fetchColumn() === 'tienda') { $bonif_edit = (int)floor($cant * 0.20); $und_edit = $cant + $bonif_edit; }
        }
        ...
        $msg_ok = "Registro <strong>#$id_v</strong> actualizado.";
    }
}
...
try {
    ...
    if ($vcd) $ventas_con_detalle = $vcd->fetchAll(PDO::FETCH_COLUMN);
} catch (Exception $e) { /* tabla aún no creada */ }

$total_ventas=0;$und_ventas=0;$n_ventas=0;$und_bonif=0;$und_consumo=0;
foreach ($registros_hoy as $r) {
    if ($r['tipo_salida']==='venta') { $total_ventas+=$r['total_venta']; $und_ventas+=$r['unidades_vendidas']; $n_ventas++; }
    elseif ($r['tipo_salida']==='bonificacion') $und_bonif++;
    else $und_consumo++;
}
...
.btn-sec{background:var(--ccard);color:var(--ink2);border:1px solid var(--border);border-radius:10px;padding:.45rem .9rem;font-size:.82rem;font-weight:600;display:inline-flex;align-items:center;gap:.4rem;text-decoration:none;transition:all .2s;cursor:pointer;font-family:inherit;}
.btn-sec:hover{background:var(--clight);border-color:var(--c3);}
.btn-sec.active{background:rgba(198,113,36,.12);border-color:var(--c3);color:var(--c3);}
.g-body{display:grid;grid-template-columns:340px 1fr;gap:.7rem;min-height:0;}
...
.fl input,.fl select{width:100%;border:1px solid var(--border);border-radius:9px;padding:.45rem .75rem;font-size:.84rem;color:var(--ink);font-family:inherit;background:var(--clight);transition:border-color .2s;box-sizing:border-box;}
...
.cat-btn{border:2px solid var(--border);border-radius:10px;padding:.55rem .5rem;text-align:center;cursor:pointer;background:var(--clight);transition:all .2s;font-family:inherit;}
.cat-btn:hover{border-color:var(--c3);}
.cat-btn.active{border-color:var(--c3);background:rgba(198,113,36,.1);box-shadow:0 0 0 3px rgba(198,113,36,.12);}
.cat-btn .cat-price{font-family:'Fraunces',serif;font-size:1.1rem;font-weight:800;color:var(--ink);}
.cat-btn.active .cat-price{color:var(--c3);}
.cat-btn .cat-stock{font-size:.6rem;font-weight:700;margin-top:.15rem;padding:.1rem .35rem;border-radius:10px;display:inline-block;}
.stk-ok{color:#2e7d32;background:rgba(46,125,50,.1);}.stk-warn{color:#e65100;background:rgba(230,81,0,.1);}.stk-zero{color:#c62828;background:rgba(198,40,40,.1);}
.fl-row{display:grid;grid-template-columns:1fr 1fr;gap:.5rem;}
.tipo-grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:.35rem;}
.tipo-btn{border:2px solid var(--border);border-radius:9px;padding:.45rem .3rem;text-align:center;cursor:pointer;background:var(--clight);transition:all .2s;font-family:inherit;font-size:.72rem;font-weight:600;color:var(--ink2);}
.tipo-btn.active{box-shadow:0 0 0 3px rgba(0,0,0,.06);}
.tipo-btn[data-tipo="venta"].active{border-color:#2e7d32;background:rgba(46,125,50,.08);color:#2e7d32;}
.tipo-btn[data-tipo="bonificacion"].active{border-color:#1565c0;background:rgba(21,101,192,.08);color:#1565c0;}
.tipo-btn[data-tipo="consumo_interno"].active{border-color:#e65100;background:rgba(230,81,0,.08);color:#e65100;}
.tipo-btn i{display:block;font-size:1.1rem;margin-bottom:.15rem;}
.total-display{background:linear-gradient(135deg,rgba(46,125,50,.08),rgba(46,125,50,.03));border:1px solid rgba(46,125,50,.2);border-radius:10px;padding:.6rem .8rem;text-align:center;margin-bottom:.5rem;}
.total-lbl{font-size:.6rem;font-weight:700;text-transform:uppercase;letter-spacing:.12em;color:#2e7d32;}
.total-val{font-family:'Fraunces',serif;font-size:1.6rem;font-weight:800;color:#1b5e20;}
.total-und{font-size:.72rem;color:#388e3c;margin-top:.1rem;}
.total-display.no-income{background:linear-gradient(135deg,rgba(21,101,192,.06),rgba(21,101,192,.02));border-color:rgba(21,101,192,.18);}
.total-display.no-income .total-lbl{color:#1565c0;}.total-display.no-income .total-val{color:#1565c0;}
.btn-guardar{width:100%;background:linear-gradient(135deg,var(--c3),var(--c1));color:#fff;border:none;border-radius:10px;padding:.7rem;font-size:.9rem;font-weight:700;cursor:pointer;font-family:inherit;box-shadow:0 4px 14px rgba(198,113,36,.3);display:flex;align-items:center;justify-content:center;gap:.4rem;transition:all .2s;margin-top:.3rem;}
...
/* Mode toggle */
.mode-toggle{display:flex;border:1px solid var(--border);border-radius:10px;overflow:hidden;margin-bottom:.7rem;}
.mode-btn{flex:1;padding:.5rem;text-align:center;font-size:.78rem;font-weight:700;cursor:pointer;border:none;background:var(--clight);color:var(--ink3);font-family:inherit;transition:all .2s;}
.mode-btn.active{background:linear-gradient(135deg,var(--c3),var(--c1));color:#fff;}
/* Catalog cards */
.price-tabs{display:flex;gap:.3rem;margin-bottom:.6rem;overflow-x:auto;padding-bottom:.2rem;}
.price-tab{padding:.4rem .7rem;border:2px solid var(--border);border-radius:9px;font-size:.78rem;font-weight:700;cursor:pointer;background:var(--clight);color:var(--ink2);white-space:nowrap;font-family:'Fraunces',serif;transition:all .2s;}
.price-tab:hover{border-color:var(--c3);}
.price-tab.active{border-color:var(--c3);background:rgba(198,113,36,.1);color:var(--c3);}
.prod-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(115px,1fr));gap:.5rem;margin-bottom:.6rem;max-height:220px;overflow-y:auto;padding:.2rem;}
.prod-card{border:1.5px solid var(--border);border-radius:11px;overflow:hidden;background:#fff;cursor:pointer;transition:all .25s;text-align:center;}
.prod-card:hover{border-color:var(--c3);box-shadow:0 3px 12px rgba(198,113,36,.12);transform:translateY(-2px);}
.prod-card.in-cart{border-color:#2e7d32;background:rgba(46,125,50,.04);}
.prod-card img{width:100%;height:80px;object-fit:cover;display:block;}
.prod-card .pc-placeholder{width:100%;height:80px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,rgba(198,113,36,.06),rgba(198,113,36,.02));font-size:2.2rem;}
.prod-card .pc-name{padding:.3rem .4rem;font-size:.7rem;font-weight:700;color:var(--ink);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.prod-card .pc-action{padding:0 .4rem .4rem;font-size:.6rem;color:var(--c3);font-weight:700;cursor:pointer;}
.prod-card.in-cart .pc-action{color:#2e7d32;}
.prod-card.expanded{border-color:var(--c3);box-shadow:0 4px 16px rgba(198,113,36,.2);transform:translateY(-2px);z-index:2;}
...
.pc-form .pf-row{display:flex;align-items:center;gap:.3rem;margin-bottom:.3rem;}
.pc-form .pf-row label{font-size:.55rem;font-weight:700;text-transform:uppercase;color:var(--ink3);min-width:32px;}
.pc-form .pf-row input{width:55px;border:1px solid var(--border);border-radius:6px;padding:.25rem;text-align:center;font-size:.82rem;font-family:'Fraunces',serif;font-weight:700;background:#fff;}
.pc-form .pf-row input:focus{outline:none;border-color:var(--c3);}
.pc-form .pf-add{width:100%;padding:.35rem;border:none;border-radius:7px;background:linear-gradient(135deg,var(--c3),var(--c1));color:#fff;font-size:.72rem;font-weight:700;cursor:pointer;font-family:inherit;display:flex;align-items:center;justify-content:center;gap:.25rem;transition:all .15s;}
.pc-form .pf-add:hover{transform:scale(1.02);box-shadow:0 2px 8px rgba(198,113,36,.3);}
/* Cart */
.cart-section{border-top:1px dashed var(--border);padding-top:.6rem;margin-top:.4rem;}
...
.cart-title .cart-badge{background:var(--c3);color:#fff;font-size:.55rem;padding:.1rem .4rem;border-radius:10px;}
...
.cart-list{display:flex;flex-direction:column;gap:.3rem;max-height:180px;overflow-y:auto;}
.cart-item{display:flex;align-items:center;gap:.4rem;padding:.4rem .5rem;border:1px solid var(--border);border-radius:9px;background:var(--clight);animation:fadeUp .2s ease;}
...
.cart-item .ci-fields{display:flex;gap:.3rem;align-items:center;}
.cart-item .ci-fields input{width:46px;border:1px solid var(--border);border-radius:6px;padding:.2rem;text-align:center;font-size:.78rem;font-family:'Fraunces',serif;font-weight:700;}
.cart-item .ci-fields input:focus{outline:none;border-color:var(--c3);}
.cart-item .ci-fields label{font-size:.48rem;font-weight:700;text-transform:uppercase;color:var(--ink3);}
.cart-item .ci-sub{font-family:'Fraunces',serif;font-size:.72rem;font-weight:700;color:#2e7d32;min-width:55px;text-align:right;}
.cart-item .ci-del{width:22px;height:22px;border-radius:5px;border:1px solid rgba(198,40,40,.2);background:none;color:#c62828;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:.65rem;flex-shrink:0;}
.cart-item .ci-del:hover{background:rgba(198,40,40,.08);}
...
.cart-total .ct-big{font-family:'Fraunces',serif;font-size:1.1rem;}
...
.bonif-row img{width:28px;height:28px;border-radius:5px;object-fit:cover;flex-shrink:0;}
.bonif-row .br-ph{width:28px;height:28px;border-radius:5px;background:rgba(21,101,192,.1);display:flex;align-items:center;justify-content:center;font-size:.7rem;flex-shrink:0;}
.bonif-row .br-name{flex:1;font-size:.72rem;font-weight:600;color:var(--ink);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0;}
.bonif-row .br-cat{font-size:.55rem;color:#64b5f6;font-weight:700;}
.bonif-row input{width:45px;border:1px solid rgba(21,101,192,.2);border-radius:5px;padding:.2rem;text-align:center;font-size:.78rem;font-family:'Fraunces',serif;font-weight:700;background:#fff;flex-shrink:0;}
.bonif-row input:focus{outline:none;border-color:#1565c0;}
/* Table */
...
.tag-tipo{font-size:.6rem;font-weight:700;padding:.15rem .45rem;border-radius:20px;display:inline-flex;align-items:center;gap:.25rem;}
.tag-venta{background:#e8f5e9;color:#2e7d32;border:1px solid #a5d6a7;}
.tag-bonif{background:#e3f2fd;color:#1565c0;border:1px solid #90caf9;}
.tag-consumo{background:#fff3e0;color:#e65100;border:1px solid #ffcc80;}
.btn-act{width:28px;height:28px;border-radius:7px;display:inline-flex;align-items:center;justify-content:center;border:1px solid var(--border);background:var(--clight);color:var(--ink3);font-size:.82rem;cursor:pointer;text-decoration:none;transition:all .15s;}
.btn-edit{color:#1565c0;background:rgba(21,101,192,.08);}.btn-edit:hover{background:#1565c0;color:#fff;}
.btn-del{color:#c62828;background:rgba(198,40,40,.06);}.btn-del:hover{background:#c62828;color:#fff;}
...
@media(max-width:900px){.page{height:auto;overflow:visible;margin-top:60px;padding:.5rem;}.g-body{grid-template-columns:1fr;}.wc-pills{gap:.35rem;}}
</style>
...
<div class="page">
  <div class="wc-banner">
    ...
    <div class="wc-pills">
      ...
      <div class="wc-pill"><div class="wc-pill-num"><?= $und_ventas ?></div><div class="wc-pill-lbl">Und.</div></div>
  ...
  <div class="topbar">
    ...
    <div class="top-actions">
      ...
      <a href="<?= APP_URL ?>/modules/recetas/variedades.php" class="btn-sec"><i class="bi bi-list-stars"></i> Variedades</a>
    </div>
  </div>
  ...
  <div class="g-body">
    ...
    <div class="card">
      <div class="ch">
        <div class="ch-left"><div class="ch-ico ico-nar"><i class="bi bi-cart-plus-fill"></i></div><span class="ch-title">Nueva salida</span></div>
      </div>
      <div class="form-body">
        ...
        <div class="mode-toggle">
          <button type="button" class="mode-btn active" id="mode-rapido" onclick="switchMode('rapido')"><i class="bi bi-lightning-charge-fill"></i> Venta rápida</button>
          <button type="button" class="mode-btn" id="mode-detalle" onclick="switchMode('detalle')"><i class="bi bi-cart4"></i> Detallar pedido</button>
        </div>
          ...
          <form method="POST" id="form-venta">
            ...
            <div class="fl">
              <div class="cat-grid">
                ...
                ?>
                <div class="cat-btn" data-id="<?= $c['id_categoria'] ?>" data-precio="<?= $c['precio_unitario'] ?>" data-stock="<?= $stk ?>" onclick="selCat(this)">
              ...
              <div style="margin-top:.4rem;">
                ...
                <div id="custom-input" style="display:none;margin-top:.35rem;">
                  <input type="number" id="inp-custom-precio" min="100" step="100" placeholder="Ej: 1500" style="width:100%;border:1px solid var(--border);border-radius:9px;padding:.5rem;font-size:.9rem;font-family:'Fraunces',serif;font-weight:700;text-align:center;background:var(--clight);" oninput="setCustomPrice()">
                </div>
              </div>
            </div>
            <div class="sec-sep">¿Cuánto?</div>
            <div class="fl-row">
              <div class="fl"><label>Cantidad</label><input type="number" id="inp-cantidad" name="cantidad" min="1" step="1" placeholder="Ej: 40" oninput="calcTotal()"></div>
              <div class="fl"><label>O monto total ($)</label><input type="number" id="inp-monto" min="1" step="1" placeholder="Ej: 20000" oninput="calcFromMonto()"></div>
            </div>
            ...
            <div class="fl"><div class="tipo-grid">
              <div class="tipo-btn active" data-tipo="venta" onclick="selTipo(this)"><i class="bi bi-cash-coin"></i> Venta</div>
              <div class="tipo-btn" data-tipo="bonificacion" onclick="selTipo(this)"><i class="bi bi-gift"></i> Bonif.</div>
              <div class="tipo-btn" data-tipo="consumo_interno" onclick="selTipo(this)"><i class="bi bi-cup-hot"></i> Consumo</div>
            </div></div>
            <div id="wrap-cliente">
            ...
            <!-- Bonificación tienda preview -->
            <div id="bonif-preview" style="display:none;background:rgba(21,101,192,.06);border:1px solid rgba(21,101,192,.18);border-radius:10px;padding:.5rem .75rem;margin-bottom:.5rem;font-size:.78rem;color:#1565c0;text-align:center;">
              🏪 Tienda: +<strong id="bonif-cant">0</strong> unidades bonificadas (20%) = <strong id="bonif-total">0</strong> entregadas
            </div>

            <div id="wrap-napa" style="display:none;">
              <div style="background:rgba(198,113,36,.05);border:1px solid rgba(198,113,36,.15);border-radius:10px;padding:.6rem .8rem;margin-bottom:.5rem;">
                <label style="display:flex;align-items:center;gap:.4rem;cursor:pointer;font-size:.8rem;font-weight:600;color:var(--ink2);">
                  <input type="checkbox" name="dar_napa" value="1" id="chk-napa" onchange="toggleNapa()" style="accent-color:var(--c3);width:16px;height:16px;">
                  🎁 Dar ñapa
                </label>
                <div id="napa-input" style="display:none;margin-top:.4rem;">
                  <input type="number" name="napa_cantidad" id="inp-napa" min="1" step="1" value="1" style="width:70px;border:1px solid var(--border);border-radius:8px;padding:.35rem .5rem;font-size:.85rem;text-align:center;font-family:'Fraunces',serif;font-weight:700;" oninput="calcTotal()">
                  <span style="font-size:.75rem;color:var(--ink3);">extra gratis</span>
                </div>
              </div>
            </div>
            <button type="submit" name="guardar_venta" class="btn-guardar"><i class="bi bi-check-lg"></i> Registrar</button>
          </form>
        </div>
        ...
        <div id="panel-detalle" style="display:none;">
          <form method="POST" id="form-pedido">
            ...
            <div id="bonif-panel" style="display:none;margin-top:.5rem;">
              <div style="background:rgba(21,101,192,.06);border:1px solid rgba(21,101,192,.18);border-radius:10px;padding:.7rem .85rem;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.5rem;">
                  <span style="font-size:.65rem;font-weight:700;text-transform:uppercase;letter-spacing:.12em;color:#1565c0;">🏪 Bonificación tienda (20%)</span>
                  <span style="font-size:.78rem;font-weight:700;color:#1565c0;">Repartir: <strong id="bonif-total-lbl">0</strong> unidades</span>
                </div>
                <div id="bonif-varieties" style="max-height:180px;overflow-y:auto;margin-bottom:.4rem;">
                  <div style="text-align:center;padding:.5rem;font-size:.75rem;color:#64b5f6;">Cargando variedades...</div>
                </div>
    ...
    <div class="card">
      <div class="ch">
        <div class="ch-left"><div class="ch-ico ico-grn"><i class="bi bi-list-ul"></i></div><span class="ch-title">Registros de hoy</span></div>
      ...
      <div class="tbl-wrap">
        <table class="gt">
          <thead><tr><th>Hora</th><th>Tipo</th><th>Categoría</th><th style="text-align:center">Und.</th><th style="text-align:right">Total</th><th>Cliente</th><th style="text-align:center">Bonif.</th><th style="text-align:center">—</th></tr></thead>
          <tbody>
          <?php
          ...
          ?>
          <tr>
            <td style="color:var(--ink3);font-size:.75rem;white-space:nowrap"><?= date('h:i a', strtotime($r['fecha_hora'])) ?></td>
            ...
            <td style="text-align:center;font-family:'Fraunces',serif;font-weight:700;"><?= $r['unidades_vendidas'] ?></td>
            <td style="text-align:right;font-weight:700;font-family:'Fraunces',serif;<?= $r['tipo_salida']!=='venta'?'color:var(--ink3)':'color:#1b5e20' ?>"><?= $r['tipo_salida']==='venta'?'$'.number_format($r['total_venta'],0,',','.'):'—' ?></td>
            <td style="font-size:.78rem;color:var(--ink3)"><?= $r['tipo_salida']==='venta'?htmlspecialchars($r['cliente']):'—' ?></td>
            <td style="text-align:center;font-size:.78rem;"><?php if ($r['bonificacion']>0): ?><?= $r['cliente']!=='Mostrador'?'<span style="color:#1565c0;font-weight:700;">+'.$r['bonificacion'].' 🏪</span>':'<span style="color:#c67124;font-weight:700;">+'.$r['bonificacion'].' 🎁</span>' ?><?php else: ?>—<?php endif; ?></td>
            <td style="text-align:center;white-space:nowrap;">
              <div style="display:flex;gap:.25rem;justify-content:center;">
                ...
                <button type="button" class="btn-act btn-edit" title="Editar" onclick="editarPedido(<?= $r['id_venta'] ?>)"><i class="bi bi-pencil"></i></button>
                <?php else: ?>
                <button type="button" class="btn-act btn-edit" title="Editar" onclick="abrirEdit(<?= $r['id_venta'] ?>,<?= $r['id_categoria_precio'] ?? 0 ?>,'<?= $r['tipo_salida'] ?>',<?= $r['unidades_vendidas'] ?>,<?= $r['id_cliente'] ?? 0 ?>)"><i class="bi bi-pencil"></i></button>
                <?php endif; ?>
                <a href="?del_venta=<?= $r['id_venta'] ?>" class="btn-act btn-del" title="Eliminar" onclick="return confirm('¿Eliminar?')"><i class="bi bi-trash3"></i></a>
              </div>
            </td>
          </tr>
          <?php endforeach; ?>
          </tbody>
          <tfoot><tr><td colspan="3" style="font-weight:700;">Total ventas</td><td style="text-align:center;font-family:'Fraunces',serif;"><?= $und_ventas ?></td><td style="text-align:right;font-family:'Fraunces',serif;font-size:1rem;color:#1b5e20;">$<?= number_format($total_ventas,0,',','.') ?></td><td colspan="3"></td></tr></tfoot>
        </table>
      </div>
      <?php endif; ?>
    </div>
...
<!-- Modal editar -->
<div id="modal-edit" style="display:none;position:fixed;inset:0;z-index:2000;background:rgba(0,0,0,.45);align-items:center;justify-content:center;">
  <div style="background:#fff;border-radius:14px;padding:1.4rem;width:90%;max-width:420px;box-shadow:0 12px 40px rgba(0,0,0,.2);">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;">
      <strong><i class="bi bi-pencil-square" style="color:var(--c3);"></i> Editar</strong>
      <button onclick="cerrarEdit()" style="background:none;border:none;font-size:1.2rem;cursor:pointer;">&times;</button>
    </div>
    <form method="POST">
      ...
      <div style="margin-bottom:.6rem;"><label style="font-size:.7rem;font-weight:700;">Categoría</label>
        <select name="ev_categoria" id="ev-cat" style="width:100%;padding:.4rem;border:1px solid var(--border);border-radius:8px;">
          <?php foreach ($categorias as $c): ?><option value="<?= $c['id_categoria'] ?>">$<?= number_format($c['precio_unitario'],0,',','.') ?></option><?php endforeach; ?>
        </select></div>
      <div style="margin-bottom:.6rem;"><label style="font-size:.7rem;font-weight:700;">Cantidad</label>
        <input type="number" name="ev_cantidad" id="ev-cant" min="1" required style="width:100%;padding:.4rem;border:1px solid var(--border);border-radius:8px;box-sizing:border-box;"></div>
      <div style="margin-bottom:.6rem;"><label style="font-size:.7rem;font-weight:700;">Tipo</label>
        <select name="ev_tipo" id="ev-tipo" style="width:100%;padding:.4rem;border:1px solid var(--border);border-radius:8px;">
          <option value="venta">Venta</option><option value="bonificacion">Bonificación</option><option value="consumo_interno">Consumo</option>
        </select></div>
      <div style="margin-bottom:.8rem;"><label style="font-size:.7rem;font-weight:700;">Cliente</label>
        <select name="ev_cliente" id="ev-cli" style="width:100%;padding:.4rem;border:1px solid var(--border);border-radius:8px;">
          <option value="0">Mostrador</option>
          <?php foreach ($clientes as $cl): ?><option value="<?= $cl['id_cliente'] ?>"><?= htmlspecialchars($cl['nombre']) ?></option><?php endforeach; ?>
        </select></div>
      <button type="submit" name="editar_venta" style="width:100%;padding:.55rem;background:var(--c3);color:#fff;border:none;border-radius:9px;font-weight:700;cursor:pointer;font-family:inherit;"><i class="bi bi-check-lg"></i> Guardar</button>
    </form>
  </div>
</div>

<!-- Modal editar pedido detallado -->
<div id="modal-edit-pedido" style="display:none;position:fixed;inset:0;z-index:2000;background:rgba(0,0,0,.5);align-items:center;justify-content:center;overflow-y:auto;">
  <div style="background:#fff;border-radius:14px;padding:1.2rem;width:95%;max-width:500px;max-height:90vh;overflow-y:auto;box-shadow:0 12px 40px rgba(0,0,0,.25);margin:1rem auto;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.8rem;">
      <strong style="font-size:1rem;"><i class="bi bi-pencil-square" style="color:var(--c3)"></i> Editar pedido detallado</strong>
      <button onclick="cerrarEditPedido()" style="background:none;border:none;font-size:1.3rem;cursor:pointer;">&times;</button>
    </div>
    <form method="POST" id="form-edit-pedido">
      ...
      <div style="font-size:.65rem;font-weight:700;text-transform:uppercase;letter-spacing:.12em;color:var(--ink3);margin-bottom:.4rem;">Selecciona precio</div>
      <div class="price-tabs" id="ep-price-tabs" style="margin-bottom:.6rem;">
        <?php foreach ($categorias as $c): ?>
        <div class="price-tab" data-id="<?= $c['id_categoria'] ?>" data-precio="<?= $c['precio_unitario'] ?>" onclick="epSelPrice(this)">
          $<?= number_format($c['precio_unitario'],0,',','.') ?>
        </div>
        <?php endforeach; ?>
      </div>
      ...
      <div style="font-size:.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.12em;color:var(--ink3);margin:.4rem 0;">🛒 Carrito <span class="cart-badge" id="ep-cart-count">0</span></div>
      <div id="ep-cart-body"><div class="cart-empty">Agrega productos</div></div>
      ...
      <div style="margin-top:.6rem;">
        <div style="font-size:.65rem;font-weight:700;text-transform:uppercase;letter-spacing:.12em;color:var(--ink3);margin-bottom:.3rem;">Cliente</div>
        <select name="edit_ped_cliente" id="ep-cliente" style="width:100%;padding:.4rem;border:1px solid var(--border);border-radius:8px;">
          <option value="0">Mostrador</option>
          <?php foreach ($clientes as $cl): ?><option value="<?= $cl['id_cliente'] ?>"><?= htmlspecialchars($cl['nombre']) ?> 🏪</option><?php endforeach; ?>
        </select>
      </div>

      <button type="submit" name="editar_pedido" class="btn-guardar" id="ep-btn-save" style="margin-top:.7rem;" disabled>
...
function setCustomPrice(){var val=parseFloat(document.getElementById('inp-custom-precio').value)||0;if(val>0){document.getElementById('inp-cat').value='';document.getElementById('inp-precio-custom').value=val;precioSel=val;stockSel=9999;calcTotal();}}
...
function selTipo(el){document.querySelectorAll('.tipo-btn').forEach(function(b){b.classList.remove('active')});el.classList.add('active');var tipo=el.dataset.tipo;document.getElementById('inp-tipo').value=tipo;document.getElementById('wrap-cliente').style.display=tipo==='venta'?'block':'none';document.getElementById('wrap-napa').style.display='none';document.getElementById('chk-napa').checked=false;document.getElementById('napa-input').style.display='none';calcTotal();}
var _updating=false;
function calcTotal(){if(_updating)return;_updating=true;var cant=parseInt(document.getElementById('inp-cantidad').value)||0;var box=document.getElementById('total-box');if(cant>0&&precioSel>0){var tipo=document.getElementById('inp-tipo').value;var total=tipo==='venta'?cant*precioSel:0;document.getElementById('total-val').textContent=tipo==='venta'?'$'+total.toLocaleString('es-CO'):'$0';document.getElementById('total-und').textContent=cant+' und × $'+precioSel.toLocaleString('es-CO');box.style.display='block';document.getElementById('inp-monto').value=cant*precioSel;var sel=document.getElementById('sel-cliente');var opt=sel.options[sel.selectedIndex];
    var bp=document.getElementById('bonif-preview');var wn=document.getElementById('wrap-napa');
    if(tipo==='venta'&&opt&&opt.dataset.tipo==='tienda'){
      var bonif=Math.floor(cant*0.20);
      document.getElementById('bonif-cant').textContent=bonif;
      document.getElementById('bonif-total').textContent=(cant+bonif);
      bp.style.display='block';wn.style.display='none';
    }else if(tipo==='venta'&&(!opt||opt.value==='0')){
      bp.style.display='none';wn.style.display='block';
    }else{bp.style.display='none';wn.style.display='none';}}else{box.style.display='none';}_updating=false;}
function calcFromMonto(){if(_updating)return;_updating=true;var monto=parseInt(document.getElementById('inp-monto').value)||0;if(precioSel>0&&monto>0){var cant=Math.floor(monto/precioSel);document.getElementById('inp-cantidad').value=cant;}var cant2=parseInt(document.getElementById('inp-cantidad').value)||0;var box=document.getElementById('total-box');if(cant2>0&&precioSel>0){var tipo=document.getElementById('inp-tipo').value;var total=tipo==='venta'?cant2*precioSel:0;document.getElementById('total-val').textContent=tipo==='venta'?'$'+total.toLocaleString('es-CO'):'$0';document.getElementById('total-und').textContent=cant2+' und × $'+precioSel.toLocaleString('es-CO');box.style.display='block';}_updating=false;}
...
function addToCartFromCard(idVar) {
  ...
  if (cant <= 0) return;
  cart.push({id_variedad: idVar, nombre: v.nombre, imagen: v.imagen, precio: currentPrice, cantidad: cant, napa: napa, catId: currentCatId});
...
function checkBonifPanel() {
  ...
  if (opt && opt.dataset.tipo === 'tienda' && cart.length > 0) {
    var totalUnd = 0;
    cart.forEach(function(item) { totalUnd += item.cantidad; });
    bonifTotal = Math.floor(totalUnd * 0.20);
    document.getElementById('bonif-total-lbl').textContent = bonifTotal;
...
function updateBonifStatus() {
  ...
  inputs.forEach(function(inp) {
    ...
    total += val;
    if (val > 0) {
      items.push({id_variedad: parseInt(inp.dataset.bonifId), cantidad: val, precio: parseFloat(inp.dataset.bonifPrecio)});
    }
  });

  var status = document.getElementById('bonif-status');
  if (total === bonifTotal) {
    status.textContent = '✅ ' + total + '/' + bonifTotal + ' unidades distribuidas';
    status.style.background = 'rgba(46,125,50,.1)';
    status.style.color = '#2e7d32';
  } else if (total > bonifTotal) {
    status.textContent = '⚠️ ' + total + '/' + bonifTotal + ' — sobran ' + (total - bonifTotal);
    status.style.background = 'rgba(198,40,40,.1)';
    status.style.color = '#c62828';
  } else {
    status.textContent = '📝 ' + total + '/' + bonifTotal + ' — faltan ' + (bonifTotal - total);
...
function epAddToCart(idVar) {
  ...
  var napa = parseInt(pcard.querySelector('.pf-napa').value) || 0;
  if (cant <= 0) return;
  epCart.push({id_variedad: idVar, nombre: v.nombre, imagen: v.imagen, precio: epCurrentPrice, cantidad: cant, napa: napa, catId: epCurrentCatId});
  pcard.classList.remove('expanded');
  pcard.classList.add('in-cart');
  pcard.querySelector('.pc-action').innerHTML = '✅ En carrito';
  epRenderCart();
...
// ══ EDIT MODAL ══
function abrirEdit(id,cat,tipo,cant,cli){document.getElementById('ev-id').value=id;document.getElementById('ev-cat').value=cat;document.getElementById('ev-cant').value=cant;document.getElementById('ev-tipo').value=tipo;document.getElementById('ev-cli').value=cli;document.getElementById('modal-edit').style.display='flex';}
