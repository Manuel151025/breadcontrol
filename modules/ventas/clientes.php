...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_cliente'])) {
    ...
    if (!$nombre) {
        ...
        try {
            $pdo->prepare("UPDATE cliente SET nombre=?, tipo='tienda', telefono=?, notas=? WHERE id_cliente=?")
                ->execute([$nombre, $telefono, $notas, $id_edit]);
        ...
        if ($ck->fetch()) {
            ...
            try {
                $pdo->prepare("INSERT INTO cliente (nombre, tipo, telefono, notas, activo) VALUES (?, 'tienda', ?, ?, 1)")
                    ->execute([$nombre, $telefono, $notas]);
