...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_link'])) {
    ...
    if ($habilitar && empty($link)) {
        ...
        $msg_err = 'La URL del link no es valida.';
    } elseif ($link && !preg_match('#^https?://([a-z0-9\-]+\.)*wompi\.co/#i', $link)) {
        $msg_err = 'La URL debe ser un link valido de Wompi (checkout.wompi.co o checkout.nequi.wompi.co).';
    } else {
        try {
            $stmt = $pdo->prepare("UPDATE configuracion SET nequi_link_pago=?, nequi_titular=?, wompi_habilitado=?, wompi_confirmar_auto=?");
            $stmt->execute([
                $link ?: null,
                $titular ?: null,
                $habilitar,
            ...
            $msg_ok = 'Configuracion de pagos guardada correctamente.';
            $config = $pdo->query("SELECT * FROM configuracion LIMIT 1")->fetch();
        } catch (Exception $e) {
