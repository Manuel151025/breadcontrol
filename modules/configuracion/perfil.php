...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_perfil'])) {
    ...
    if (empty($nombre)) { $msg_err = 'El nombre es obligatorio.'; }
    elseif ($correo && !filter_var($correo, FILTER_VALIDATE_EMAIL)) { $msg_err = 'El correo no es válido.'; }
    else {
        $pdo->prepare("UPDATE usuario SET nombre_completo=?, correo_electronico=?, telefono=? WHERE id_usuario=?")
            ->execute([$nombre, $correo ?: null, $telefono ?: null, $user['id_usuario']]);
        $msg_ok = "Datos actualizados correctamente.";
        $stmt->execute([$user['id_usuario']]); $datos = $stmt->fetch();
    }
    $tab_activo = 'datos';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cambiar_clave'])) {
    ...
    if (!password_verify($actual, $datos['contrasena_hash'])) { $msg_err = 'Contraseña actual incorrecta.'; }
    elseif (strlen($nueva) < 6) { $msg_err = 'Mínimo 6 caracteres.'; }
    elseif ($nueva !== $conf) { $msg_err = 'Las contraseñas no coinciden.'; }
    else {
        $pdo->prepare("UPDATE usuario SET contrasena_hash=? WHERE id_usuario=?")->execute([password_hash($nueva, PASSWORD_BCRYPT), $user['id_usuario']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_pin'])) {
    ...
    if (!password_verify($clave_p, $datos['contrasena_hash'])) { $msg_err = 'Contraseña incorrecta.'; }
    elseif (!preg_match('/^\d{6}$/', $pin)) { $msg_err = 'El PIN debe ser de 6 dígitos.'; }
    else {
        $pdo->prepare("UPDATE usuario SET pin_recuperacion=? WHERE id_usuario=?")->execute([password_hash($pin, PASSWORD_BCRYPT), $user['id_usuario']]);
