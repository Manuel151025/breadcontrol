...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['agregar_variedad'])) {
    ...
    if (empty($nombre) || !$id_cat) {
        ...
        $chk = $pdo->prepare("SELECT id_variedad FROM variedad_pan WHERE nombre=? AND id_categoria_precio=? AND activo=1");
        $chk->execute([$nombre, $id_cat]);
        if ($chk->fetch()) {
            ...
            if (!$msg_err) {
                $pdo->prepare("INSERT INTO variedad_pan (id_categoria_precio, nombre, imagen) VALUES (?,?,?)")
                    ->execute([$id_cat, $nombre, $img_path]);
...
// ── Editar variedad ──
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['editar_variedad'])) {
    ...
    if ($id_var && $nombre) {
        ...
        if ($update_img) {
            $pdo->prepare("UPDATE variedad_pan SET nombre=?, imagen=? WHERE id_variedad=?")->execute([$nombre, $img_path, $id_var]);
        } else {
            $pdo->prepare("UPDATE variedad_pan SET nombre=? WHERE id_variedad=?")->execute([$nombre, $id_var]);
        }
...
if (!empty($_GET['del_var'])) {
    $pdo->prepare("UPDATE variedad_pan SET activo=0 WHERE id_variedad=?")->execute([(int)$_GET['del_var']]);
    header('Location: variedades.php'); exit;
}

$categorias = $pdo->query("SELECT * FROM categoria_precio WHERE activo=1 ORDER BY precio_unitario")->fetchAll();
$variedades = $pdo->query("SELECT v.*, cp.nombre AS cat_nombre, cp.precio_unitario FROM variedad_pan v INNER JOIN categoria_precio cp ON cp.id_categoria=v.id_categoria_precio WHERE v.activo=1 ORDER BY cp.precio_unitario, v.nombre")->fetchAll();

$var_por_cat = [];
foreach ($variedades as $v) $var_por_cat[$v['id_categoria_precio']][] = $v;
